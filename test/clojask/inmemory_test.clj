...
(deftest df-api-test
  (testing "Single dataframe manipulation APIs"
    ...
    (is (= clojask.classes.DataFrame.DataFrame (type (compute y 8 "test/clojask/test_outputs/tmp.csv" :in-memory true))))))
...
(deftest join-api-test
  (testing "Join dataframes APIs"
    ...
    (is (= clojask.classes.DataFrame.DataFrame (type (compute (left-join x y ["Employee"] ["Employee"]) 8 "test/clojask/test_outputs/join-api.csv" :exception false :in-memory true))))
    (is (= clojask.classes.DataFrame.DataFrame (type (compute (right-join x y ["Employee"] ["Employee"]) 8 "test/clojask/test_outputs/join-api.csv" :exception false :in-memory true))))
    (is (= clojask.classes.DataFrame.DataFrame (type (compute (inner-join x y ["Employee"] ["Employee"]) 8 "test/clojask/test_outputs/join-api.csv" :exception false :in-memory true))))
    (is (= clojask.classes.DataFrame.DataFrame (type (compute (rolling-join-forward x y ["Employee"] ["Employee"] "Salary" "Salary") 8 "test/clojask/test_outputs/join-api.csv" :exception false :in-memory true))))))
