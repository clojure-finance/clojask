...
(def wide "test/clojask/melt.csv")
(def long "test/clojask/dcast.csv")
...
(deftest melt-wide-to-long
  (let [out "test/clojask/test_outputs/melt.csv"]
    (rs/melt (ck/dataframe wide) out ["family_id" "age_mother"] ["dob_child1" "dob_child2" "dob_child3"])
    (is (= (first (lines long)) (first (lines out))) "header names the id columns, measure and value")
    (is (same-table? (lines long) (lines out)))))
...
(deftest dcast-long-to-wide
  (let [out "test/clojask/test_outputs/dcast.csv"]
    (rs/dcast (ck/dataframe long) out ["family_id" "age_mother"] "measure" "value" ["dob_child1" "dob_child2" "dob_child3"])
    (is (same-table? (lines wide) (lines out)))))

(deftest dcast-renamed-value-columns
  (let [out "test/clojask/test_outputs/dcast-named.csv"]
    (rs/dcast (ck/dataframe long) out ["family_id" "age_mother"] "measure" "value" ["dob_child1" "dob_child3"]
...
(deftest compute-header-collection
  (testing "compute :header with a collection replaces the column names"
    (let [out "test/clojask/test_outputs/header.csv"]
      (ck/compute (ck/dataframe wide) 1 out :select ["family_id" "age_mother"] :header ["id" "age"])
